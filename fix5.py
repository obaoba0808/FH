...
def fix_file(path, get_sections_fn, schema_old_new, get_faq_ld, date_check_marker):
    ...
    original_body = src[open_end:close_pos]
    already_has_new = date_check_marker in original_body
    h2_starts = [mm.start() for mm in re.finditer(r'<h2>', original_body)]
    
    # Strategy: include FAQ section (which comes after H2 九)
    # If there are more than 9 H2s (including FAQ H2), keep up to 9th H2
    # But ALWAYS include the FAQ section (which has its own H2 + details)
    # The FAQ section starts with <h2>常見問題 FAQ</h2> and ends with </details>
    faq_h2_pos = original_body.find('<h2>常見問題 FAQ</h2>')
    
    if faq_h2_pos >= 0:
        # FAQ H2 exists - keep through the end of FAQ (include all 6 details items)
        # Find the last </details> in the body (end of FAQ)
        last_details_pos = original_body.rfind('</details>')
        if last_details_pos >= 0:
            correct_body = original_body[:last_details_pos + len('</details>')]
            print(f"Including FAQ section through </details>: {len(correct_body)} chars")
        else:
            correct_body = original_body[:h2_starts[9]] if len(h2_starts) >= 10 else original_body
    elif len(h2_starts) >= 10:
        correct_body = original_body[:h2_starts[9]]
    else:
        correct_body = original_body

    h2_in_correct = correct_body.count('<h2>')
    print(f"  Body: {len(correct_body)} chars, {h2_in_correct} H2, already_has_new={already_has_new}")

    if already_has_new:
        final_body = correct_body
    else:
        final_body = correct_body + '\n' + get_sections_fn()
        print(f"  Added new sections: {len(final_body)} chars")

    cta_pos = src.find('<section class="py-12 px-6 relative z-10 max-w-4xl mx-auto mb-12">')
    before_open = src[:m_open.start()]
    after_close = src[close_pos + len('</main>'):]
    
    new_main_block = f'{open_tag}\n{final_body}\n    </main>\n'
    new_src = before_open + new_main_block + after_close
    ...
    # Update JSON-LD
    if schema_old_new:
        old_s, new_s = schema_old_new
        if old_s in new_src:
            new_src = new_src.replace(old_s, new_s)
            print(f"  Schema updated (literal)")
        else:
            compact_old = re.sub(r'\s+', '', old_s)
            if compact_old in new_src:
                new_src = new_src.replace(compact_old, re.sub(r'\s+', '', new_s))
                print(f"  Schema updated (compact)")
            else:
                pat = re.compile(r'<script type="application/ld\+json>.*?</script>', re.DOTALL)
                new_src = pat.sub(new_s, new_src)
                print(f"  Schema updated (regex)")
    elif get_faq_ld:
        faq_match = re.search(r'("@type":\s*"FAQPage".*?"mainEntity":\s*\[.*?\]\s*\})', new_src, re.DOTALL)
        if faq_match:
            new_src = new_src.replace(faq_match.group(0), get_faq_ld())
            print(f"  FAQ JSON-LD updated")

    with open(path, 'w', encoding='utf-8') as f:
        f.write(new_src)
    print(f"  Written {len(new_src)} chars")
    ...
    oe = mo.end()
    cp = c.find('</main>', oe)
    body = c[oe:cp] if cp >= 0 else ''
    h2 = body.count('<h2>')
    faq = body.count('<details')
    date_ok = '"dateModified": "2026-07-15"' in c
    faq_q = c.count('"@type": "Question"')
    cn = sum(1 for ch in body if '\u4e00' <= ch <= '\u9fff' or '\u3400' <= ch <= '\u4dbf')
    print(f'\n=== {path.split("/")[-1]} ===')
    print(f'  H2={h2}, FAQ HTML={faq}, JSON-LD Q={faq_q}, dateModified={date_ok}, CN={cn}\n')
...
SPECIAL_SCHEMA ='''<script type="application/ld+json>
{
...
SPECIAL_OLD_SCHEMA = '<script type="application/ld+json>\n{\n  "@context": "https://schema.org", "@type": "NewsArticle",\n  "headline": "八大行業大解密：酒店與傳播到底差在哪？",\n  "image": ["https://ibb.co/5hVhkRwL"]\n}\n</script>'

fix_file(
    r'C:\Users\FH01\.qclaw\workspace-cwapojim0yfmyvq8\FH-website\how_much.html',
    lambda: SEC_HOWMUCH,
    None,  # no schema replacement
    lambda: FAQ_LD_HOWMUCH,
    '四、影響收費的關鍵變數'
)

fix_file(
    r'C:\Users\FH01\.qclaw\workspace-cwapojim0yfmyvq8\FH-website\special_industries.html',
    lambda: SPECIAL_SECTIONS,
    (SPECIAL_OLD_SCHEMA, SPECIAL_SCHEMA),
    None,
    '四、酒店與傳播的核心差異：場地與機動性'
)
