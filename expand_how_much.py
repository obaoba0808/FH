...
# ── 4) Insert new sections before Global CTA ────────────────────
marker = '    </main>\n    <!-- ========================================== -->\n    <!-- 以上區塊為每篇文章需要替換的內容 -->\n    <!-- ========================================== -->\n\n    <!-- Global CTA -->'
replacement = '    </main>\n    <!-- ========================================== -->\n    <!-- 以上區塊為每篇文章需要替換的內容 -->\n    <!-- ========================================== -->\n' + new_sections + '\n    <!-- Global CTA -->'
...
faq_count = content.count('<details class="mb-4')
json_ld_updated = '"dateModified": "2026-07-15"' in content

# Extract article body text (between <main ...> and </main>)
import re as re2
m = re2.search(r'<main[^>]*>(.*?)</main>', content, re2.DOTALL)
article_text = m.group(1) if m else ''
# Strip tags, count Chinese characters
chinese = re2.sub(r'<[^>]+>', '', article_text)
cn_chars = sum(1 for c in chinese if '\u4e00' <= c <= '\u9fff')
cn_chars += sum(1 for c in article_text if '\u4e00' <= c <= '\u9fff')

# Simpler: count all Chinese chars in article body
body_text = re2.sub(r'<[^>]+>', ' ', article_text)
cn_chars = sum(1 for c in body_text if '\u4e00' <= c <= '\u9fff')

print(f"  H2 count      : {h2_count} (need 9+)")
print(f"  FAQ items     : {faq_count} (need 6+)")
print(f"  dateModified  : {json_ld_updated}")
print(f"  Chinese chars : {cn_chars} (need 1500+)")
