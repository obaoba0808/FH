...
def fix_howmuch():
    ...
    # Find <main> opening tag
    m_open = re.search(r'<main[^>]*>', src)
    if not m_open:
        print("ERROR: no <main> opening tag")
        return
    
    open_tag = m_open.group(0)  # e.g. <main class="...">
    open_end = m_open.end()       # position right after the opening tag

    # Find </main> closing tag
    close_pos = src.find('</main>', open_end)
    if close_pos == -1:
        print("ERROR: no </main> closing tag")
        return
    
    # Original body content (between tags)
    original_body = src[open_end:close_pos]
    print(f"Original body: {len(original_body)} chars, H2={original_body.count('<h2>')}")

    # Check if new sections already in body
    already_has_new = '四、影響收費的關鍵變數' in original_body
    
    # Find all H2 start positions
    h2_starts = [mm.start() for mm in re.finditer(r'<h2>', original_body)]
    
    # Keep first 9 H2 sections (cut at start of 10th H2 if it exists)
    if len(h2_starts) >= 10:
        cutoff = h2_starts[9]
        correct_body = original_body[:cutoff]
        print(f"Trimmed to first 9 H2: {len(correct_body)} chars")
    else:
        correct_body = original_body
        print(f"No trim needed, {len(h2_starts)} H2")

    # Build final body
    if already_has_new:
        final_body = correct_body
    else:
        final_body = correct_body + '\n' + get_howmuch_sections()
    ...
    if cta_pos == -1:
        print("ERROR: Global CTA not found")
        return

    # Reconstruct:
    # before_open: everything before <main...>
    before_open = src[:m_open.start()]
    # after_close: everything after </main>
    after_close = src[close_pos + len('</main>'):]
    
    # New main block: opening tag + new body + closing tag
    ...
    # Update FAQ JSON-LD using regex
    faq_match = re.search(r'("@type":\s*"FAQPage".*?"mainEntity":\s*\[.*?\]\s*\})', new_src, re.DOTALL)
    if faq_match:
        new_src = new_src.replace(faq_match.group(0), get_howmuch_faq_ld())
        print("FAQ JSON-LD updated")
    else:
        print("FAQ JSON-LD: regex not matched")
...
def fix_special():
    ...
    m_open = re.search(r'<main[^>]*>', src)
    if not m_open:
        print("ERROR: no <main> opening tag in special")
        return
    ...
    close_pos = src.find('</main>', open_end)
    if close_pos == -1:
        print("ERROR: no </main> in special")
        return
    
    original_body = src[open_end:close_pos]
    print(f"Original body: {len(original_body)} chars, H2={original_body.count('<h2>')}")

    already_has_new = '四、酒店與傳播的核心差異：場地與機動性' in original_body
    
    h2_starts = [mm.start() for mm in re.finditer(r'<h2>', original_body)]
    print(f"H2 count: {len(h2_starts)}")
    ...
    new_src = schema_pat.sub(get_special_schema(), new_src)
...
def verify(path):
    ...
    # Find first <main...> opening tag
    m_open = re.search(r'<main[^>]*>', c)
    if not m_open:
        print(f"  VERIFY: No <main> found!")
        return
    
    open_end = m_open.end()
    close_pos = c.find('</main>', open_end)
    if close_pos == -1:
        print(f"  VERIFY: No </main> found!")
        return
    
    body = c[open_end:close_pos]
    h2 = body.count('<h2>')
