# -*- coding: utf-8 -*-
"""
Final fix for both files:
1. Move content currently outside </main> to inside <main>
2. Ensure correct dateModified and FAQ JSON-LD
"""
import re
...
# ─── HOW_MUCH.HTML ──────────────────────────────────────────────
with open(r'C:\Users\FH01\.qclaw\workspace-cwapojim0yfmyvq8\FH-website\how_much.html', encoding='utf-8') as f:
    src = f.read()

# Step 1: Extract content inside <main>...</main>
m_main = re.search(r'(<main[^>]*>)(.*?)(</main>)', src, re.DOTALL)
if not m_main:
    print("ERROR: <main> not found in how_much.html")
else:
    open_tag = m_main.group(1)
    old_body = m_main.group(2)
    after_main = src[m_main.end():]  # everything after </main>
    
    # Check if new sections already in file (check entire file, not just body)
    already_done = '四、影響收費的關鍵變數：時段、地點、包廂人數' in src and old_body.count('<h2>') >= 9
    
    if already_done:
        print("how_much.html: sections already present, skipping body insert")
    else:
        new_body = old_body + '\n' + SEC_HOWMUCH
        new_src = open_tag + new_body + '\n    </main>\n' + after_main
        with open(r'C:\Users\FH01\.qclaw\workspace-cwapojim0yfmyvq8\FH-website\how_much.html', 'w', encoding='utf-8') as f:
            f.write(new_src)
        print("how_much.html: body updated")

# Step 2: Update dateModified
with open(r'C:\Users\FH01\.qclaw\workspace-cwapojim0yfmyvq8\FH-website\how_much.html', encoding='utf-8') as f:
    src = f.read()
src = re.sub(r'"dateModified":\s*"[^"]*"', '"dateModified": "2026-07-15"', src)

# Step 3: Update FAQ JSON-LD
old_faq_ld = r'''{
          "@type": "FAQPage",
          ...
          "mainEntity": [
            {
              ...
              "acceptedAnswer": {
                "@type": "Answer",
                "text": "在台北正統傳播公司，行情約為每小時 NT$1,200 至 NT$2,500 不等，取決於等級（公關、保三、VIP）。歐巴傳播主打透明定價、絕不事後加價。"
              }
            }
          ]
        }'''
...
if old_faq_ld in src:
    src = src.replace(old_faq_ld, new_faq_ld)
    print("how_much.html: FAQ JSON-LD updated")
else:
    # Try single-line variant
    old_faq_ld2 = '{"@type":"FAQPage","@id":"https://obaoba0808.github.io/FH/how_much.html#faq","mainEntity":[{"@type":"Question","name":"叫傳播多少錢？","acceptedAnswer":{"@type":"Answer","text":"在台北正統傳播公司，行情約為每小時 NT$1,200 至 NT$2,500 不等，取決於等級（公關、保三、VIP）。歐巴傳播主打透明定價、絕不事後加價。"}}]}'
    if old_faq_ld2 in src:
        src = src.replace(old_faq_ld2, new_faq_ld.replace('\n', '').replace('          ', ''))
        print("how_much.html: FAQ JSON-LD updated (compact)")
    else:
        print("how_much.html: FAQ JSON-LD old pattern not found!")
...
m_main = re.search(r'(<main[^>]*>)(.*?)(</main>)', src, re.DOTALL)
if not m_main:
    print("ERROR: <main> not found in special_industries.html")
else:
    open_tag = m_main.group(1)
    old_body = m_main.group(2)
    after_main = src[m_main.end():]
    
    already_done = '四、酒店與傳播的核心差異：場地與機動性' in src and old_body.count('<h2>') >= 9
    
    if already_done:
        print("special_industries.html: sections already present, skipping body insert")
    else:
        new_body = old_body + '\n' + SEC_SPECIAL
        new_src = open_tag + new_body + '\n    </main>\n' + after_main
        with open(r'C:\Users\FH01\.qclaw\workspace-cwapojim0yfmyvq8\FH-website\special_industries.html', 'w', encoding='utf-8') as f:
            f.write(new_src)
        print("special_industries.html: body updated")

# Step 2: Update dateModified + schema
with open(r'C:\Users\FH01\.qclaw\workspace-cwapojim0yfmyvq8\FH-website\special_industries.html', encoding='utf-8') as f:
    src = f.read()

# Replace old NewsArticle schema with full Article + FAQPage graph
...
if old_schema in src:
    ...
    print("special_industries.html: schema updated")
else:
    # Try without newlines
    old_schema2 = old_schema.replace('\n', '')
    if old_schema2 in src:
        src = src.replace(old_schema2, new_schema.replace('\n', ''))
        print("special_industries.html: schema updated (compact)")
    else:
        print("special_industries.html: old schema not found! Showing context...")
        idx = src.find('NewsArticle')
        if idx >= 0:
...
for fname in ['how_much.html', 'special_industries.html']:
    with open(f'C:/Users/FH01/.qclaw/workspace-cwapojim0yfmyvq8/FH-website/{fname}', encoding='utf-8') as f:
        c = f.read()
    m = re2.search(r'<main[^>]*>(.*?)</main>', c, re2.DOTALL)
