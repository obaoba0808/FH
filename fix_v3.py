...
def fix_file(filepath, faq_json_ld, is_special=False):
    with open(filepath, encoding='utf-8') as f:
        src = f.read()

    # ── Special industries content ──
    if is_special:
        new_sections = '''
...
'''
    ...
    # ── Update dateModified ──
    src = src.replace('"dateModified": "2026-04-23"', '"dateModified": "2026-07-15"')
    src = src.replace('"dateModified": "2026-07-15"', '"dateModified": "2026-07-15"')  # already updated is fine

    # ── Update JSON-LD FAQ ──
    if faq_json_ld:
        old_ld = faq_json_ld['old']
        new_ld = faq_json_ld['new']
        src = src.replace(old_ld, new_ld)

    # ── Insert new_sections AFTER </main> (inside the main tag) ──
    # Find </main> and insert right after it
    main_close = '</main>'
    idx_main = src.rfind(main_close)  # use rfind to find the article's </main>
    if idx_main == -1:
        print(f"ERROR: </main> not found in {filepath}")
        return

    # Insert new_sections right after </main>
    insert_at = idx_main + len(main_close)
    src = src[:insert_at] + '\n' + new_sections + src[insert_at:]
...
fix_file(r'C:\Users\FH01\.qclaw\workspace-cwapojim0yfmyvq8\FH-website\how_much.html', how_ld, is_special=False)
...
new_schema = r'''<script type="application/ld+json">
{
...
fix_file(r'C:\Users\FH01\.qclaw\workspace-cwapojim0yfmyvq8\FH-website\special_industries.html',
         {'old': old_schema, 'new': new_schema}, is_special=True)
