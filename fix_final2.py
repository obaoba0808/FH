...
def fix_both():
    for fname, sections, schema_repl, faq_ld, new_marker in [
        ('how_much.html', SEC_HOWMUCH, None, FAQ_LD, '四、影響收費的關鍵變數'),
        ('special_industries.html', SEC_SPECIAL, (SPECIAL_OLD, SPECIAL_NEW), None, '四、酒店與傳播的核心差異：場地與機動性'),
    ]:
        path = f'C:/Users/FH01/.qclaw/workspace-cwapojim0yfmyvq8/FH-website/{fname}'
        with open(path, encoding='utf-8') as f:
            src = f.read()

        m_open = re.search(r'<main[^>]*>', src)
        open_tag = m_open.group(0)
        open_end = m_open.end()
        close_pos = src.find('</main>', open_end)
        body = src[open_end:close_pos]

        already_has_new = new_marker in body
        h2_starts = [mm.start() for mm in re.finditer(r'<h2>', body)]

        # Find FAQ section: <h2>常見問題 FAQ</h2>
        faq_h2_pos = body.find('<h2>常見問題 FAQ</h2>')
        if faq_h2_pos >= 0:
            # FAQ exists - keep through last </details>
            last_det = body.rfind('</details>')
            if last_det >= 0:
                correct_body = body[:last_det + len('</details>')]
                print(f'{fname}: including FAQ section, {len(correct_body)} chars')
            else:
                correct_body = body[:h2_starts[9]] if len(h2_starts) >= 10 else body
        elif len(h2_starts) >= 10:
            correct_body = body[:h2_starts[9]]
        else:
            correct_body = body

        final_body = correct_body if already_has_new else (correct_body + '\n' + sections)
        print(f'  final_body: {len(final_body)} chars, H2={final_body.count("<h2>")}')

        cta_pos = src.find('<section class="py-12 px-6 relative z-10 max-w-4xl mx-auto mb-12">')
        new_src = src[:m_open.start()] + f'{open_tag}\n{final_body}\n    </main>\n' + src[close_pos + len('</main>'):]

        new_src = re.sub(r'"dateModified":\s*"[^"]*"', '"dateModified": "2026-07-15"', new_src)

        if schema_repl:
            old_s, new_s = schema_repl
            if old_s in new_src:
                new_src = new_src.replace(old_s, new_s)
                print(f'  Schema (literal)')
            else:
                pat = re.compile(r'<script type="application/ld\+json>.*?</script>', re.DOTALL)
                new_src = pat.sub(new_s, new_src)
                print(f'  Schema (regex)')
        elif faq_ld:
            m = re.search(r'("@type":\s*"FAQPage".*?"mainEntity":\s*\[.*?\]\s*\})', new_src, re.DOTALL)
            if m:
                new_src = new_src.replace(m.group(0), faq_ld)
                print(f'  FAQ JSON-LD (regex)')
        ...
        with open(path, encoding='utf-8') as f:
            c = f.read()
        mo = re.search(r'<main[^>]*>', c)
        cp = c.find('</main>', mo.end())
        bd = c[mo.end():cp] if cp >= 0 else ''
        h2 = bd.count('<h2>')
        faq = bd.count('<details')
        date_ok = '"dateModified": "2026-07-15"' in c
        faq_q = c.count('"@type": "Question"')
        cn = sum(1 for ch in bd if '\u4e00' <= ch <= '\u9fff' or '\u3400' <= ch <= '\u4dbf')
        print(f'\n=== {fname} === H2={h2} FAQ={faq} Q={faq_q} date={date_ok} CN={cn}\n')
...
SEC_HOWMUCH = '''
        ...
        <details class="mb-4 border border-white/10 rounded-xl overflow-hidden">
            ...
            <div class="px-6 py-4 text-gray-300 border-t border-white/10"><p>公關等級約 NT$1,200 – 1,400/小時，保三等級約 NT$1,500 – 1,900/小時，VIP 等級 NT$2,000 以上。實際價格以預約當下有空班的女孩與公司報價為準。</p></div>
        </details>
        <details class="mb-4 border border-white/10 rounded-xl overflow-hidden">
            <summary class="px-6 py-4 cursor-pointer font-bold text-white hover:bg-white/5 transition-colors">Q2：車馬費怎麼算？</summary>
            <div class="px-6 py-4 text-gray-300 border-t border-white/10"><p>市區（台北市、新北市核心區）一般不收車馬費；跨縣市或偏遠地區可能酌收 NT$300 – NT$800，預約時會先告知，不會到場才追加。</p></div>
        </details>
        <details class="mb-4 border border-white/10 rounded-xl overflow-hidden">
            <summary class="px-6 py-4 cursor-pointer font-bold text-white hover:bg-white/5 transition-colors">Q3：不滿意可以打槍換人嗎？</summary>
            <div class="px-6 py-4 text-gray-300 border-t border-white/10"><p>可以。在公關坐下確認服務前，若長相或氣質不符合期待，可直接要求更換，不收取任何費用。這是正規傳播公司保障消費者權益的基本機制。</p></div>
        </details>
        <details class="mb-4 border border-white/10 rounded-xl overflow-hidden">
            <summary class="px-6 py-4 cursor-pointer font-bold text-white hover:bg-white/5 transition-colors">Q4：傳播跟酒店的價格差多少？</summary>
            <div class="px-6 py-4 text-gray-300 border-t border-white/10"><p>一場四小時的酒店聚會，帳單輕鬆落在 NT$20,000 – 40,000（含框費、包廂費、酒水費等）。傳播同樣四小時、兩位公關的費用約 NT$9,600 – 14,400，節省幅度達 40% – 60%。</p></div>
        </details>
        <details class="mb-4 border border-white/10 rounded-xl overflow-hidden">
            <summary class="px-6 py-4 cursor-pointer font-bold text-white hover:bg-white/5 transition-colors">Q5：有沒有隱藏費用或加價項目？</summary>
            <div class="px-6 py-4 text-gray-300 border-t border-white/10"><p>正規公司如歐巴傳播，預約時完整告知所有費用項目（鐘點費與車馬費），絕無開桌費、經紀費、清潔費等隱藏項目。建議選擇透明報價的正規公司，避免落入低價陷阱。</p></div>
        </details>
        <details class="mb-4 border border-white/10 rounded-xl overflow-hidden">
            <summary class="px-6 py-4 cursor-pointer font-bold text-white hover:bg-white/5 transition-colors">Q6：2026 年行情是否有變動？</summary>
            <div class="px-6 py-4 text-gray-300 border-t border-white/10"><p>受基本工資調漲影響，2026 年行情較 2025 年微幅上漲約 5% – 8%。即便如此，傳播整體費用仍比傳統酒店低了近一半，是目前台北夜生活CP值最高的選擇。</p></div>
        </details>
...
FAQ_LD = r'''{
          "@type": "FAQPage",
          "@id": "https://obaoba0808.github.io/FH/how_much.html#faq",
          "mainEntity": [
            {"@type": "Question", "name": "叫傳播多少錢？", "acceptedAnswer": {"@type": "Answer", "text": "公關等級約 NT$1,200 – 1,400/小時，保三等級約 NT$1,500 – 1,900/小時，VIP 等級 NT$2,000 以上。實際價格以預約當下有空班的女孩與公司報價為準。"}},
            {"@type": "Question", "name": "車馬費怎麼算？", "acceptedAnswer": {"@type": "Answer", "text": "市區（台北市、新北市核心區）一般不收車馬費；跨縣市或偏遠地區可能酌收 NT$300 – NT$800，預約時會先告知，不會到場才追加。"}},
            {"@type": "Question", "name": "不滿意可以打槍換人嗎？", "acceptedAnswer": {"@type": "Answer", "text": "可以。在公關坐下確認服務前，若長相或氣質不符合期待，可直接要求更換，不收取任何費用。這是正規傳播公司保障消費者權益的基本機制。"}},
            {"@type": "Question", "name": "傳播跟酒店的價格差多少？", "acceptedAnswer": {"@type": "Answer", "text": "一場四小時的酒店聚會，帳單輕鬆落在 NT$20,000 – 40,000（含框費、包廂費、酒水費等）。傳播同樣四小時、兩位公關的費用約 NT$9,600 – 14,400，節省幅度達 40% – 60%。"}},
            {"@type": "Question", "name": "有沒有隱藏費用或加價項目？", "acceptedAnswer": {"@type": "Answer", "text": "正規公司如歐巴傳播，預約時完整告知所有費用項目（鐘點費與車馬費），絕無開桌費、經紀費、清潔費等隱藏項目。建議選擇透明報價的正規公司，避免落入低價陷阱。"}},
            {"@type": "Question", "name": "2026 年行情是否有變動？", "acceptedAnswer": {"@type": "Answer", "text": "受基本工資調漲影響，2026 年行情較 2025 年微幅上漲約 5% – 8%。即便如此，傳播整體費用仍比傳統酒店低了近一半，是目前台北夜生活CP值最高的選擇。"}}
          ]
        }'''
...
SEC_SPECIAL = '''
        ...
        <details class="mb-4 border border-white/10 rounded-xl overflow-hidden">
            ...
            <div class="px-6 py-4 text-gray-300 border-t border-white/10"><p>傳播是指傳播公司派遣公關小姐到客人指定的地點（如KTV、汽車旅館、私人會所）提供陪伴服務，採鐘點計費，無包廂費與酒水低消；酒店則需要客人到店消費，有高昂包廂費與酒水費，且受限於店家營業時間。</p></div>
        </details>
        <details class="mb-4 border border-white/10 rounded-xl overflow-hidden">
            ...
            <div class="px-6 py-4 text-gray-300 border-t border-white/10"><p>公關等級適合一般聚會，活潑大方，收費 NT$1,200/小時起；保三等級顏值與身材較高，適合商務場合，收費 NT$1,500/小時起；VIP模特等級為網紅或高顏值，適合重要應酬，收費 NT$2,000/小時以上。</p></div>
        </details>
        <details class="mb-4 border border-white/10 rounded-xl overflow-hidden">
            ...
            <div class="px-6 py-4 text-gray-300 border-t border-white/10"><p>傳播的工作時間更彈性，可兼顧正職或學業；地點由客人指定，女孩不需長期待在高壓的酒店環境；酒精壓力較小，不必被強迫推銷或陪同大量飲酒的飯局；收入以時計，不受幹部抽成剝削。</p></div>
        </details>
        <details class="mb-4 border border-white/10 rounded-xl overflow-hidden">
            ...
            <div class="px-6 py-4 text-gray-300 border-t border-white/10"><p>傳播的定位處於法律灰色地帶。正規傳播公司的業務本質是「陪同陪伴服務」，而非性交易，但市場上確實存在打著傳播名號實則從事非法交易的業者。選擇有固定公司制度與客服窗口的正規傳播，是保障雙方權益的關鍵。</p></div>
        </details>
        <details class="mb-4 border border-white/10 rounded-xl overflow-hidden">
            ...
            <div class="px-6 py-4 text-gray-300 border-t border-white/10"><p>歐巴傳播定位為「介於酒店與正當娛樂之間的頂級外派陪伴服務」，提供透明定價、看過滿意才計時的保障，並配有一對一客服窗口。無隱藏費用、無事後加價，是我們對消費者的核心承諾。</p></div>
        </details>
        <details class="mb-4 border border-white/10 rounded-xl overflow-hidden">
            <summary class="px-6 py-4 cursor-pointer font-bold text-white hover:bg-white/5 transition-colors">Q6：叫傳播適合哪些場合？</summary>
            <div class="px-6 py-4 text-gray-300 border-t border-white/10"><p>傳播的機動性極高，適合幾乎所有需要女性陪伴的社交場合：KTV歡唱、公司聚餐後續攤、商務飯局撐場面、私人派對、汽車旅館聚會、一對一約會等。場所由您決定，時間由您掌控，遠比酒店更有彈性。</p></div>
        </details>

'''

SPECIAL_OLD = '<script type="application/ld+json>\n{\n  "@context": "https://schema.org", "@type": "NewsArticle",\n  "headline": "八大行業大解密：酒店與傳播到底差在哪？",\n  "image": ["https://ibb.co/5hVhkRwL"]\n}\n</script>'
SPECIAL_NEW ='''<script type="application/ld+json>
{
  ...
  "@graph": [
    {
      ...
      "publisher": {"@type": "Organization", "name": "歐巴傳播", "logo": {"@type": "ImageObject", "url": "https://obaoba0808.github.io/FH/images/android-chrome-512x512.png"}}
    },
    {
      "@type": "FAQPage",
