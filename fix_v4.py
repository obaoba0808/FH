...
def fix_file_v4(filepath, new_sections_html, faq_ld_old_new=None):
    ...
    # ── Update FAQ JSON-LD ──
    if faq_ld_old_new:
        old_ld, new_ld = faq_ld_old_new['old'], faq_ld_old_new['new']
        src = src.replace(old_ld, new_ld)

    # ── Fix article body: find <main...>...</main> and replace content ──
    # Pattern: <main class="..."> ... </main>
    # Capture opening tag attributes so we can reconstruct properly
    m = re.search(r'(<main[^>]*>)(.*?)(</main>)', src, re.DOTALL)
    if not m:
        print(f"ERROR: <main> not found in {filepath}")
        return

    open_tag = m.group(1)   # e.g. <main class="...">
    old_body = m.group(2)   # content between <main> and </main>
    close_tag = m.group(3)  # </main>

    # Remove any accidentally duplicated new content already inside main
    # (If old_body already has sections 四-九, don't re-add)
    if '四、影響收費的關鍵變數' in old_body or '四、酒店與傳播的核心差異' in old_body:
        print(f"  Content already present, skipping body replacement")
    else:
        new_body = old_body + '\n' + new_sections_html

    new_src = open_tag + new_body + close_tag + src[m.end():]
    ...
    print(f'=== {name} ===')
    print(f'  H2={h2}, FAQ HTML={faq}, FAQ JSON-LD Q={faq_q}, dateModified={date_ok}')
    print(f'  Chinese chars in article body: {cn}')
    print()


# ──────────────────────────────────────────────────────────────────────
# Shared new sections content
# ──────────────────────────────────────────────────────────────────────
  ...
  "@graph": [
    {
      ...
      "publisher": {
        ...
        "logo": {
          "@type": "ImageObject",
          "url": "https://obaoba0808.github.io/FH/images/android-chrome-512x512.png"
        }
      }
    },
    {
      ...
      "mainEntity": [
        {
          ...
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "傳播是指傳播公司派遣公關小姐到客人指定的地點（如KTV、汽車旅館、私人會所）提供陪伴服務，採鐘點計費，無包廂費與酒水低消；酒店則需要客人到店消費，有高昂包廂費與酒水費，且受限於店家營業時間。"
          }
        },
        {
          "@type": "Question",
          "name": "公關、保三、模特三個等級有何差異？",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "公關等級適合一般聚會，活潑大方，收費 NT$1,200/小時起；保三等級顏值與身材較高，適合商務場合，收費 NT$1,500/小時起；VIP模特等級為網紅或高顏值，適合重要應酬，收費 NT$2,000/小時以上。"
          }
        },
        {
          "@type": "Question",
          "name": "女生入行八大行業，為什麼優先選擇傳播而非酒店？",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "傳播的工作時間更彈性，可兼顧正職或學業；地點由客人指定，女孩不需長期待在高壓的酒店環境；酒精壓力較小，不必被強迫推銷或陪同大量飲酒的飯局；收入以時計，不受幹部抽成剝削。"
          }
        },
        {
          "@type": "Question",
          "name": "傳播是否屬於八大行業？法律上如何定義？",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "傳播的定位處於法律灰色地帶。正規傳播公司的業務本質是「陪同陪伴服務」，而非性交易，但市場上確實存在打著傳播名號實則從事非法交易的業者。選擇有固定公司制度與客服窗口的正規傳播，是保障雙方權益的關鍵。"
          }
        },
        {
          "@type": "Question",
          "name": "歐巴傳播和其他傳播公司有什麼不同？",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "歐巴傳播定位為「介於酒店與正當娛樂之間的頂級外派陪伴服務」，提供透明定價、看過滿意才計時的保障，並配有一對一客服窗口。無隱藏費用、無事後加價，是我們對消費者的核心承諾。"
          }
        },
        {
          "@type": "Question",
          ...
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "傳播的機動性極高，適合幾乎所有需要女性陪伴的社交場合：KTV歡唱、公司聚餐後續攤、商務飯局撐場面、私人派對、汽車旅館聚會、一對一約會等。場所由您決定，時間由您掌控，遠比酒店更有彈性。"
          }
        }
      ]
    }
...
# ── special_industries.html sections ──
sec_special = '''
...
fix_file_v4(r'C:\Users\FH01\.qclaw\workspace-cwapojim0yfmyvq8\FH-website\how_much.html', sec_howmuch, faq_howmuch)
fix_file_v4(r'C:\Users\FH01\.qclaw\workspace-cwapojim0yfmyvq8\FH-website\special_industries.html', sec_special, faq_special)
