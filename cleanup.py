...
1. Extract correct article body (original 3 H2 + new 6 H2 once, inside <main>)
2. Remove all duplicate content outside <main>
3. Update dateModified and FAQ JSON-LD
"""
import re

# ─── CONTENT: New sections to add ──────────────────────────────
SEC_HOWMUCH = '''
          ...
          "mainEntity": [
            {
              ...
              "acceptedAnswer": {
                "@type": "Answer",
                "text": "公關等級約 NT$1,200 – 1,400/小時，保三等級約 NT$1,500 – 1,900/小時，VIP 等級 NT$2,000 以上。實際價格以預約當下有空班的女孩與公司報價為準。"
              }
            },
            {
              "@type": "Question",
              ...
              "acceptedAnswer": {
                "@type": "Answer",
                "text": "市區（台北市、新北市核心區）一般不收車馬費；跨縣市或偏遠地區可能酌收 NT$300 – NT$800，預約時會先告知，不會到場才追加。"
              }
            },
            {
              "@type": "Question",
              ...
              "acceptedAnswer": {
                "@type": "Answer",
                "text": "可以。在公關坐下確認服務前，若長相或氣質不符合期待，可直接要求更換，不收取任何費用。這是正規傳播公司保障消費者權益的基本機制。"
              }
            },
            {
              "@type": "Question",
              ...
              "acceptedAnswer": {
                "@type": "Answer",
                "text": "一場四小時的酒店聚會，帳單輕鬆落在 NT$20,000 – 40,000（含框費、包廂費、酒水費等）。傳播同樣四小時、兩位公關的費用約 NT$9,600 – 14,400，節省幅度達 40% – 60%。"
              }
            },
            {
              "@type": "Question",
              ...
              "acceptedAnswer": {
                "@type": "Answer",
                "text": "正規公司如歐巴傳播，預約時完整告知所有費用項目（鐘點費與車馬費），絕無開桌費、經紀費、清潔費等隱藏項目。建議選擇透明報價的正規公司，避免落入低價陷阱。"
              }
            },
            {
              "@type": "Question",
              ...
              "acceptedAnswer": {
                "@type": "Answer",
                "text": "受基本工資調漲影響，2026 年行情較 2025 年微幅上漲約 5% – 8%。即便如此，傳播整體費用仍比傳統酒店低了近一半，是目前台北夜生活CP值最高的選擇。"
              }
            }
          ]
        }'''
...
def fix_how_much():
    ...
    # Step 1: Extract the correct article body from what's currently in the file
    # The file has correct content inside <main> (10 H2: original 3 + new 6, plus some duplicates)
    # We need to extract just the first 9 H2 sections
    m = re.search(r'<main[^>]*>(.*?)</main>', src, re.DOTALL)
    if not m:
        print("ERROR: no main tag")
        return
    body = m.group(1)

    # Extract the correct article content:
    # Find all H2 blocks in order
    h2_positions = [(m2.start(), m2.group(0)) for m2 in re.finditer(r'<h2>', body)]
    print(f"Found {len(h2_positions)} H2 tags in body")

    # Keep first 9 H2 sections (original 3 + new 6)
    if len(h2_positions) >= 9:
        # The 9th H2 is at position 8
        cutoff = h2_positions[8][0]
        correct_body = body[:cutoff]
        # Remove anything after the 9th H2 section (extra duplicates)
        print(f"Trimming body from {len(body)} to {len(correct_body)} chars")
    else:
        correct_body = body
        print(f"Only {len(h2_positions)} H2s found, keeping all")

    # Step 2: Find where the Global CTA section starts (after </main>)
    # The Global CTA section is the <section class="py-12 px-6 relative z-10 max-w-4xl mx-auto mb-12">
    cta_section_start = src.find('<section class="py-12 px-6 relative z-10 max-w-4xl mx-auto mb-12">')
    if cta_section_start == -1:
        print("ERROR: Global CTA section not found")
        return
    print(f"Global CTA at position {cta_section_start}")

    # Step 3: Extract everything after the Global CTA section
    after_cta = src[cta_section_start:]

    # Step 4: Build new article body with correct content
    new_article_body = correct_body + '\n' + SEC_HOWMUCH

    # Step 5: Reconstruct the file
    # Extract everything before <main> (navbar, head, etc.)
    before_main = src[:m.start()]
    new_src = before_main + '\n        <main class="relative z-10 max-w-3xl mx-auto px-6 py-16 article-content">\n' + new_article_body + '\n    </main>\n' + after_cta

    # Step 6: Update dateModified using regex
    new_src = re.sub(r'"dateModified":\s*"[^"]*"', '"dateModified": "2026-07-15"', new_src)

    # Step 7: Update FAQ JSON-LD
    # Find and replace the FAQPage section in JSON-LD
    faq_match = re.search(r'"@type":\s*"FAQPage".*?"mainEntity":\s*\[.*?\]\s*\}', new_src, re.DOTALL)
    if faq_match:
        old_faq = faq_match.group(0)
        new_src = new_src.replace(old_faq, FAQ_LD_HOWMUCH.strip())
        print("FAQ JSON-LD updated")
    else:
        # Try the literal string from original file
        old_faq_literal = '{"@type":"FAQPage","@id":"https://obaoba0808.github.io/FH/how_much.html#faq","mainEntity":[{"@type":"Question","name":"叫傳播多少錢？","acceptedAnswer":{"@type":"Answer","text":"在台北正統傳播公司，行情約為每小時 NT$1,200 至 NT$2,500 不等，取決於等級（公關、保三、VIP）。歐巴傳播主打透明定價、絕不事後加價。"}}]}'
        if old_faq_literal in new_src:
            new_src = new_src.replace(old_faq_literal, FAQ_LD_HOWMUCH.replace('\n','').replace('          ',''))
            print("FAQ JSON-LD updated (compact)")
        else:
            print("FAQ JSON-LD: old pattern not found")

    with open(path, 'w', encoding='utf-8') as f:
        f.write(new_src)
    print(f"how_much.html written, {len(new_src)} chars")
    ...
    print(f'how_much.html: H2={h2}, FAQ={faq}, JSON-LD Q={faq_q}, dateModified={date_ok}, CN={cn}')


def fix_special():
    path = r'C:\Users\FH01\.qclaw\workspace-cwapojim0yfmyvq8\FH-website\special_industries.html'
    with open(path, encoding='utf-8') as f:
        src = f.read()

    # Step 1: Extract the correct article body (first 9 H2 sections)
    m = re.search(r'<main[^>]*>(.*?)</main>', src, re.DOTALL)
    if not m:
        print("ERROR: no main tag in special_industries.html")
        return
    body = m.group(1)

    h2_positions = [(m2.start(), m2.group(0)) for m2 in re.finditer(r'<h2>', body)]
    print(f"Found {len(h2_positions)} H2 tags in special body")

    if len(h2_positions) >= 9:
        cutoff = h2_positions[8][0]
        correct_body = body[:cutoff]
        print(f"Trimming special body from {len(body)} to {len(correct_body)} chars")
    else:
        correct_body = body

    # Step 2: Find Global CTA section
    cta_section_start = src.find('<section class="py-12 px-6 relative z-10 max-w-4xl mx-auto mb-12">')
    if cta_section_start == -1:
        print("ERROR: Global CTA not found in special_industries.html")
        return

    after_cta = src[cta_section_start:]

    # Step 3: Build new article body
    SEC_SPECIAL = '''
...
'''
    ...
    # Step 5: Replace old NewsArticle schema with Article + FAQPage
    ...
    new_schema ='''<script type="application/ld+json>
{
  ...
  "@graph": [
    ...
    new_src = old_schema_pattern.sub(new_schema, new_src)
    ...
    faq_q = c.count('"@type": "Question"')
    cn = sum(1 for ch in body2 if '\u4e00' <= ch <= '\u9fff' or '\u3400' <= ch <= '\u4dbf')
    print(f'special_industries.html: H2={h2}, FAQ={faq}, JSON-LD Q={faq_q}, dateModified={date_ok}, CN={cn}')
