  ...
  "@graph": [
    ...
    },
    {
      ...
      "mainEntity": [
        ...
        if not m:
            print(f"ERROR: no <main> in {fname}")
            continue

        body = m.group(1)
        body_h2 = body.count('<h2>')
        already_has_new = '四、酒店與傳播的核心差異：場地與機動性' in body or '四、影響收費的關鍵變數：時段、地點、包廂人數' in body

        # Find all H2 start positions
        h2_starts = [mm.start() for mm in re.finditer(r'<h2>', body)]
        print(f"{fname}: body has {body_h2} H2, already_has_new={already_has_new}")

        if len(h2_starts) >= 9:
            # Cut at end of 9th H2 section
            # Find the START of the 10th H2 (or end of body if fewer)
            if len(h2_starts) >= 10:
                cutoff = h2_starts[9]  # start of 10th H2
            else:
                cutoff = len(body)  # no 10th H2, take all
            correct_body = body[:cutoff]
            print(f"  Keeping first 9 H2 sections ({len(correct_body)} chars)")
        else:
            correct_body = body
            print(f"  Keeping all {len(h2_starts)} H2 sections ({len(correct_body)} chars)")

        # Build final body
        if fname == 'how_much.html':
            new_sections = get_howmuch_sections()
        else:
            new_sections = get_special_sections()
        ...
        if cta_pos == -1:
            print(f"ERROR: Global CTA not found in {fname}")
            continue
        ...
        # Update JSON-LD
        if faq_ld_old and faq_ld_new:
            compact_old = re.sub(r'\s+', '', faq_ld_old)
            compact_new = re.sub(r'\s+', '', faq_ld_new)
            if compact_old in new_src:
                new_src = new_src.replace(compact_old, compact_new)
                print(f"  FAQ JSON-LD updated (compact)")
            elif faq_ld_old in new_src:
                new_src = new_src.replace(faq_ld_old, faq_ld_new)
                print(f"  FAQ JSON-LD updated (literal)")
            else:
                print(f"  FAQ JSON-LD: old not found")
        if schema_old and schema_new:
            if schema_old in new_src:
                new_src = new_src.replace(schema_old, schema_new)
                print(f"  Schema updated (literal)")
            else:
                compact_old2 = re.sub(r'\s+', '', schema_old)
                if compact_old2 in new_src:
                    new_src = new_src.replace(compact_old2, re.sub(r'\s+', '', schema_new))
                    print(f"  Schema updated (compact)")
                else:
                    # Use regex
                    pat = re.compile(r'<script type="application/ld\+json>.*?</script>', re.DOTALL)
                    new_src = pat.sub(schema_new, new_src)
