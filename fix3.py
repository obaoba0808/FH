...
def fix_howmuch():
    ...
    already_has_new = '四、影響收費的關鍵變數' in body
    print(f"how_much.html: body has {body_h2} H2, already_has_new={already_has_new}")

    h2_starts = [mm.start() for mm in re.finditer(r'<h2>', body)]
    print(f"  H2 positions: {h2_starts}")

    if len(h2_starts) >= 10:
        # Keep H2 0-8 (first 9 H2), cut at start of H2[9]
        cutoff = h2_starts[9]
        correct_body = body[:cutoff]
        print(f"  Trimmed to first 9 H2 at pos {cutoff}")
    else:
        correct_body = body
        print(f"  No trimming needed, {len(h2_starts)} H2 total")
    ...
    # Update FAQ JSON-LD using regex (more robust)
    faq_match = re.search(r'("@type":\s*"FAQPage".*?"mainEntity":\s*\[.*?\]\s*\})', new_src, re.DOTALL)
    if faq_match:
        old_faq = faq_match.group(1)
        new_faq = get_howmuch_faq_ld()
        new_src = new_src.replace(old_faq, new_faq)
        print("  FAQ JSON-LD updated (regex)")
    else:
...
def fix_special():
    ...
    print(f"special_industries.html: body has {body_h2} H2, already_has_new={already_has_new}")

    h2_starts = [mm.start() for mm in re.finditer(r'<h2>', body)]

    # Keep all H2 (up to 9th or all if fewer)
    if len(h2_starts) >= 10:
        cutoff = h2_starts[9]
        correct_body = body[:cutoff]
    elif len(h2_starts) >= 9:
        # 9th H2 exists - keep up to but NOT including the 9th H2 (since it might be an extra duplicate)
        # Actually, we want to keep through 9th H2. Since there's no 10th, take all.
        correct_body = body
    else:
        correct_body = body

    print(f"  Body length: {len(correct_body)} chars, {correct_body.count('<h2>')} H2")
    ...
    new_schema = get_special_schema()
    new_src = schema_pat.sub(new_schema, new_src)
